...
export class CreateProductComponent implements OnInit{
  ...
  imageUrl: any;
  constructor(private productsService: ProductsService, private route: ActivatedRoute, private store: Store<any>) {
    this.CategoriesObrsv$ = this.store.pipe(select('category')
  ...
  ngOnInit() {
    ...
    .subscribe(
      (res: any) => {
        ...
        this.store.dispatch(new CategoriesActions.initializeCategoriesAction(this.categories))
      }
    )
...
handleFileInput(event: any): void {
  ...
  if (this.fileToUpload.length > remainingSlots) {
    ...
    remainingSlots === 0? this.errorMessage =  `Your limit is 3 images.` : this.errorMessage = this.errorMessage
    return;
  }
